...
import { MessageSquare, Menu, Loader2, LogOut } from 'lucide-react';
...
const ChatDashboard = () => {
      ...
      try {
        ...
        if (currentSession) {
          ...
          if (currentSession.selectedModel) {
            navigate(`/chat/${sessionId}/model/${currentSession.selectedModel}`, { replace: true });
            return;
          }
        }
  ...
  const handleSendPrompt = async (newPrompt) => {
    ...
    try {
      ...
      if (!selectedModel) {
        setMultiResponses(null);
        const responses = await sendInitialPrompt(newPrompt, activeSessionId, history);
        ...
        const response = await sendContinuePrompt(newPrompt, selectedModel, history, activeSessionId);
  ...
  const handleSelectModel = async (modelId, responseContent) => {
    ...
    try {
      ...
      navigate(`/chat/${sessionId}/model/${modelId}`);
    } catch (error) {
      console.error(error);
